// Package credstore stores API tokens in the OS keychain or config file.
package credstore
...
const (
	...
	Service            = "familyjourney-cli"
	EnvDisableKeychain = "FAMILYJOURNEY_DISABLE_KEYCHAIN"
	EnvStorage         = "FAMILYJOURNEY_STORAGE"
)
...
func KeychainAvailable() bool {
	...
	_, err := active.Get(Service, "__familyjourney_keychain_probe__")
...
func ResolveBackend(storage string) (string, error) {
	...
	case BackendKeychain:
		if !KeychainAvailable() {
			return "", fmt.Errorf("keychain backend requested but no OS keyring is available on %s (set FAMILYJOURNEY_STORAGE=file or FAMILYJOURNEY_DISABLE_KEYCHAIN=1)", runtime.GOOS)
		}
